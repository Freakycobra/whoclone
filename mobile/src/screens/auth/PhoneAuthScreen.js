...
export default function PhoneAuthScreen({ navigation }) {
  ...
  const [selectedCode, setSelectedCode] = useState(COUNTRY_CODES[1]); // India default
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      // Demo mode — skip real API for now
      setStep('otp');
    } finally {
      setLoading(false);
    }
  ...
  const verifyOTP = async () => {
    if (otp.length < 4) {
      Alert.alert('Invalid', 'Enter the OTP sent to your phone');
      return;
    }
    setLoading(true);
    try {
      ...
      // Demo mode — go to profile setup
      setToken('demo_token_123');
      setUser({ id: '1', phone: `${selectedCode.code}${phone}`, coins: 100, isVip: false });
      navigation.replace('ProfileSetup');
    } finally {
      setLoading(false);
    }
