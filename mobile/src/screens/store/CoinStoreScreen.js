...
import { COIN_PACKS, SUBSCRIPTION_PLANS, VIP_PERKS } from '../../constants';
import { useAuthStore } from '../../store/authStore';

const { width } = Dimensions.get('window');

// Detect country (simplified — in production use IP geolocation)
const getUserCountry = () => 'IN'; // Default to India for demo

export default function CoinStoreScreen({ navigation }) {
  const { user, addCoins } = useAuthStore();
  const [activeTab, setActiveTab] = useState('coins'); // 'coins' | 'vip'
  const [selectedPlan, setSelectedPlan] = useState('monthly');

  const country = getUserCountry();
  const packs = COIN_PACKS[country] || COIN_PACKS.default;
  const plans = SUBSCRIPTION_PLANS[country] || SUBSCRIPTION_PLANS.default;
  const currency = packs[0]?.currency || '$';

  const handlePurchase = (pack) => {
    // In production: trigger Google Play IAP
    Alert.alert(
  ...
  const handleVIPPurchase = (plan) => {
    Alert.alert(
      'Get VIP',
      `${plan.label}: ${plan.currency || '$'}${plan.price}/${plan.period}`,
      [
        { text: 'Cancel', style: 'cancel' },
