...
import { VIP_PERKS, SUBSCRIPTION_PLANS } from '../../constants';

const { width } = Dimensions.get('window');

export default function PremiumScreen({ navigation }) {
  const [selectedPlan, setSelectedPlan] = useState('monthly');
  const plans = SUBSCRIPTION_PLANS.IN; // auto-detect in production
  const currency = '₹';
          ...
          <TouchableOpacity activeOpacity={0.85}>
            ...
            >
              <Text style={styles.ctaBtnText}>
                👑 Get VIP — {currency}{plans[selectedPlan]?.price}/{plans[selectedPlan]?.period}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
